...
public:
   ...
   * @param extended_size if true, maximum supported message length is 65518 bytes, else 239 bytes. Must be setup with
   * the same size on both the sender and the reciving side. Reason for having this is for backward compatibility with
   * existing users who expects the size to be 239 bytes.
   */
...
public:
   ...
   * @param length size of the message. Maximum supported message length is 65518 bytes if extended_size is true in
   * constructor, else if false, 239 bytes.
   * @return the encrypted message. This will be larger then the original message. Empty vector on failure.
   */
